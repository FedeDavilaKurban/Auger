def read_config(config_file):
    ...
    params = {
        ...
        'azimuth_max': config.getfloat('Parameters', 'azimuth_max'),
        'random_generation': config.get('Parameters', 'random_generation')
    }
...
def crosscorrelations(data, events_a8, randoms_gxs, randoms_auger, params, treecorr_config, write_corr=True, clusters=None):
    ...
    import treecorr
    import matplotlib.pyplot as plt 
    from generate_names import get_filecorrname
...
def main():
    ...
    import numpy as np
    from astropy.io import ascii
    from generate_randoms import generate_RandomCatalogue, generate_CR_like_randoms, generate_exposure_filtered_randoms
    from generate_names import get_filecorrname, get_filename, get_skyplotname, get_corrplotname
    ...
    if params['random_generation'] not in ['exposure_filtered', 'CR_like', \
                                           'parabol', '2D_KDE', 'uniform']:
        raise ValueError(f"Unknown random generation method: {params['random_generation']}")
    
    # Read UHECR data
    events_a8 = ascii.read('../data/Auger/events_a8_lb.dat')
    events_a8 = events_a8[events_a8['dec'] < params['dec_max']] # Cut declination
    if params['azimuth_max'] not in [None, '', ' ', 'None']:
        events_a8 = events_a8[(events_a8['azimuth'] < params['azimuth_max']) & \
                              (events_a8['azimuth'] > -params['azimuth_max'])]
    ...
    if params['random_generation'] == 'uniform':
        randoms_auger = generate_RandomCatalogue(len(events_a8), params, nmult = 100)
    elif params['random_generation'] == 'parabol':
        randoms_auger = generate_RandomCatalogue(len(events_a8), params, nmult = 100, dec=events_a8['dec'])  # Generate randoms for Auger events
    elif params['random_generation'] == '2D_KDE':
        randoms_auger = generate_RandomCatalogue(len(events_a8), params, nmult = 100, dec=events_a8['dec'], \
                                                 ra=events_a8['RA'])
    elif params['random_generation'] == 'exposure_filtered':
        randoms_auger = generate_exposure_filtered_randoms(len(events_a8)*100, params, plot=True)
    elif params['random_generation'] == 'CR_like':
        randoms_auger = generate_CR_like_randoms(len(events_a8), 100, events_a8)
